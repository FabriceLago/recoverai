-- Deterministic, explainable Risk Engine (see docs §13-14).
-- calculate_risk_score is the single source of truth: used by a trigger to
-- snapshot history into risk_scores, and by opportunity_risk_scores to serve
-- always-current values without needing a scheduled recompute job.

create function calculate_risk_score(opp opportunities)
returns jsonb
language plpgsql
stable
as $$
...
-- Snapshot risk into history (drives the opportunity Timeline in a later phase).
create function sync_risk_score()
returns trigger
language plpgsql
as $$
declare
  result jsonb;
begin
  result := calculate_risk_score(new);
  insert into risk_scores (
    ...
    (result ->> 'score')::int,
    result ->> 'risk_level',
    (result ->> 'time_score')::int,
    (result ->> 'value_score')::int,
    (result ->> 'engagement_score')::int,
    (result ->> 'deadline_score')::int,
    result -> 'explanation'
  );
  return new;
end;
...
create trigger opportunities_sync_risk_score
  after insert or update on opportunities
  for each row execute function sync_risk_score();

-- Always-current risk per opportunity (no staleness between snapshots); security_invoker
-- means it enforces the querying user's RLS on the underlying opportunities table.
create view opportunity_risk_scores
with (security_invoker = true) as
select
  o.id as opportunity_id,
  o.organization_id,
  (r.result ->> 'score')::int as score,
  r.result ->> 'risk_level' as risk_level,
  (r.result ->> 'time_score')::int as time_score,
  (r.result ->> 'value_score')::int as value_score,
  (r.result ->> 'engagement_score')::int as engagement_score,
  (r.result ->> 'deadline_score')::int as deadline_score,
  r.result -> 'explanation' as explanation
from opportunities o
cross join lateral (select calculate_risk_score(o) as result) r;
